...
// Verifies Account signatures and every purpose/audience/installation/time binding before exposing a directive.
public sealed class PrivacyDirectiveVerifier : IDisposable
{
    ...
    // Returns one bounded domain command and neutral failure text; raw tokens are never logged or persisted.
    public bool TryVerify(string token, DateTimeOffset now, out PlayerDataErasureDirective? directive)
    {
        directive = null;
        ...
        try
        {
            ...
            if (root.GetProperty("iss").GetString() != "pocketrankings-account" || root.GetProperty("aud").GetString() != "pocketrankings-player_profile" || root.GetProperty("action").GetString() != "person.player_data_erasure_requested.v1" || root.GetProperty("installation_key").GetString() != installationKey) return false;
            ...
            directive = new(root.GetProperty("request_id").GetGuid(), root.GetProperty("person_id").GetGuid(), root.GetProperty("jti").GetGuid(), issued);
            return true;
        }
